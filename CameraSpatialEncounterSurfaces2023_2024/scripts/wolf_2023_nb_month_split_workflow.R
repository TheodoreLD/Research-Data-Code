###############################################################################
# Wolf relative encounter frequency: 2023 INLA-SPDE ordered science workflow
# -----------------------------------------------------------------------------
# Purpose
#   Standalone 2023 workflow for modelling relative wolf encounter frequency
...
#   Final 2023 model: negative-binomial likelihood,
...
#
# Key corrections used in this final 2023 workflow
#   * Uses joint posterior samples for PPC, PIT, fitted counts, and CV.
#   * Uses camera-level and camera-month-row-level validation summaries.
...
#   This v2 2023 script uses the configured final NB spatial-month model.
#   It still runs model comparison against Poisson/NB/ZINB candidates, a prior-influence
...
OUTPUT_DIR <- Sys.getenv(
  "WOLF_OUTPUT_DIR",
  unset = file.path(PROJECT_DIR, "outputs", "2023", "wolf_2023_NB_month_split_final_v1")
)
...
settings <- list(
  ...
  prior_sigma = c(2.00, 0.05),        # final 2023 prior: P(sigma > 2.00) = 0.05
...
# Intercept prior mean is filled after loading the 2023 data, using the crude
...
# Zero inflation: skeptical prior for the ZINB comparison model.
# Center: 5% structural-zero probability; SD = 1.5 on logit scale.
PRIOR_ZI_LOGIT_MEAN <- qlogis(0.05)
PRIOR_ZI_LOGIT_PREC <- 1 / 1.5^2

# Negative-binomial size prior for the final NB likelihood. Smaller size = stronger overdispersion.
...
load_2023_survey <- function(settings) {
  dep <- readr::read_csv(path_in("deployments_2023.csv"), show_col_types = FALSE)
  obs <- readr::read_csv(path_in("observations_2023.csv"), show_col_types = FALSE)
  ...
  stop_missing_columns(dep, required_dep, "[wolf_2023] deployments")
  stop_missing_columns(obs, required_obs, "[wolf_2023] observations")
  ...
  if (!nrow(deployments)) stop("[wolf_2023] no valid dated deployments.")
  ...
  if (missing_event_time > 0) {
    stop("[wolf_2023] cannot split events by month: ",
  ...
  cat(sprintf(
    "[wolf_2023] cameras %d | model rows %d | positive rows %d | events %d | effort %.1f camera-days | observed %.2f /100\n",
    nrow(camera_rate),
    nrow(model_dat),
  ...
  cat(sprintf(
    "[wolf_2023] month effect: reference=%s | prediction=%s | months=%s\n",
...
write_diagnostic_plots <- function(diag, camera_sf) {
  ...
  obs_fit_plot <- ggplot(camera_diag, aes(fitted_count, y)) +
    ...
    labs(
      title = "Observed vs fitted camera counts: wolf_2023",
  ...
  residual_plot <- ggplot(residual_sf) +
    ...
    coord_sf(datum = NA) +
    labs(title = "Spatial Pearson residuals: wolf_2023",
         subtitle = FINAL_MODEL_NAME,
  ...
  pit_row_plot <- ggplot(data.frame(pit = diag$row_pit), aes(pit)) +
    ...
    labs(
      title = "Posterior predictive PIT, model rows: wolf_2023",
  ...
  pit_camera_plot <- ggplot(data.frame(pit = diag$camera_pit), aes(pit)) +
    ...
    geom_hline(yintercept = 1, linetype = 2, colour = "red") +
    labs(
      title = "Posterior predictive PIT, camera aggregates: wolf_2023",
      subtitle = sprintf("mean %.3f | KS p %.3g",
  ...
  if (nrow(variogram)) {
    variogram_plot <- ggplot(variogram, aes(dist, gamma)) +
      ...
      labs(
        title = "Residual semivariogram: wolf_2023",
        subtitle = FINAL_MODEL_NAME,
        x = "distance (m)",
        y = "semivariance"
      ) +
  ...
  effort_plot <- ggplot(residual_covariate,
    ...
    geom_smooth(se = FALSE, method = "loess", formula = y ~ x) +
    labs(title = "Residuals vs effort: wolf_2023",
  ...
  month_plot <- ggplot(residual_covariate,
    ...
    geom_hline(yintercept = 0, linetype = 2) +
    labs(title = "Residuals by calendar month: wolf_2023",
  ...
  lat_plot <- ggplot(residual_covariate,
    ...
    geom_hline(yintercept = 0, linetype = 2) +
    geom_smooth(se = FALSE, method = "loess", formula = y ~ x) +
    labs(title = "Residuals vs latitude: wolf_2023",
...
write_annualization_weights <- function(fit, model_dat, settings) {
  ...
  list(
    factor = factor,
    label = sprintf(
      "annualized 2023 surface; baseline %s; factor %.3f",
      prediction_month,
      factor
    ),
    weights = month_effort
...
fit_2023_model <- function(model_dat, settings, family) {
  cat(sprintf("\n[wolf_2023] fitting %s: family=%s\n", FINAL_MODEL_NAME, family))
  ...
  cat(sprintf("[wolf_2023] prediction cells: %d at %.0f m\n",
...
fit_2023_diagnostic_model <- function(model_dat, settings, family) {
  cat(sprintf("\n[wolf_2023] fitting observed-data diagnostic model for posterior sampling\n"))
...
spatial_block_cv <- function(model_dat, settings, family, K = CV_K) {
  cat(sprintf("\n[wolf_2023] spatial block CV for %s (K=%d)\n", FINAL_MODEL_NAME, K))
  ...
  if (K_final < 2) {
    cat("[wolf_2023] spatial CV skipped: too few cameras.\n")
    return(NULL)
  }
  ...
  for (f in sort(unique(fold))) {
    cat(sprintf("[wolf_2023]   CV fold %s\n", f))
  ...
  if (length(failed_folds)) {
    stop("[wolf_2023] spatial CV failed for fold(s): ",
         paste(failed_folds, collapse = "; "))
  }
  if (!length(rows)) stop("[wolf_2023] spatial CV produced no successful folds.")
  ...
  cat(sprintf(
    "[wolf_2023] spatial CV rows: mean LPD %.3f | RMSE count %.2f | RMSE rate %.2f | 90%% coverage %.2f\n",
  ...
  cat(sprintf(
    "[wolf_2023] spatial CV cameras: RMSE count %.2f | RMSE rate %.2f | 90%% coverage %.2f\n",
...
write_validation_report <- function(model_dat, diag, cv, prediction,
  ...
  report <- c(
    ...
    if (!is.null(annualization)) {
      sprintf("  Map aggregation: effort-weighted annualized 2023 surface; scale factor %.3f.",
    ...
    "  Spatial surface is month-adjusted and annualized over the sampled 2023 months."
  )
...
write_manifest <- function(model_dat, diag, cv) {
  ...
  readr::write_csv(manifest, path_out("wolf_2023_run_manifest.csv"))
  invisible(manifest)
}
...
fit_observed_model_generic <- function(model_dat, settings, family,
  ...
  if (!isTRUE(spatial)) {
    cat(sprintf("[wolf_2023] model comparison fit: %s | family=%s | spatial=false\n",
                model_label, family))
    fit <- INLA::inla(
  ...
  cat(sprintf("[wolf_2023] model comparison fit: %s | family=%s | spatial=true\n",
              model_label, family))
...
run_model_comparison <- function(model_dat, settings) {
  ...
  cat("\n[wolf_2023] running model comparison\n")
  specs <- data.frame(
    model = c(
  ...
  report <- c(
    "Model comparison:",
    "  Purpose: check whether the selected spatial NB model is supported relative to simpler Poisson/NB alternatives and whether overdispersion or zero inflation is needed.",
...
run_mesh_sensitivity <- function(model_dat, settings, family) {
  ...
  cat("\n[wolf_2023] running mesh sensitivity\n")
...
run_temporal_bin_model_sensitivity <- function(model_dat,
  ...
  cat(sprintf(
    "\n[wolf_2023] running temporal bin sensitivity with %d posterior samples per variant\n",
  ...
  for (bd in unique(as.integer(bin_days_values))) {
    label <- if (bd == 7L) {
      "NB_spatial_weekly_timebin"
    } else if (bd == 14L) {
      "NB_spatial_biweekly_timebin"
    } else {
      paste0("NB_spatial_", bd, "day_timebin")
    }
    ...
    res <- tryCatch({
      ...
      fit_obj <- fit_2023_diagnostic_model(timebin_dat, settings, family)
...
write_scientific_limitations_report <- function(model_dat) {
  ...
  lines <- c(
    ...
    "   Calendar camera-month is included as a fixed effect after splitting effort by month and assigning events by eventStart month.",
    "   The final map is an effort-weighted annualized 2023 encounter-frequency surface, not a single-month map.",
...
write_workflow_order_report <- function() {
  lines <- c(
    "Ordered 2023 spatial-modelling workflow:",
    "",
...
write_exploratory_checks <- function(model_dat) {
  cat("\n[wolf_2023] writing exploratory checks\n")
  ...
  raw_map <- ggplot(camera_sf) +
    ...
    coord_sf(datum = NA) +
    labs(title = "Observed wolf encounter frequency by camera: wolf_2023",
  ...
  date_plot <- ggplot(model_dat, aes(start_date, wolf_events_per_100_days)) +
    ...
    geom_smooth(se = TRUE, method = "loess", formula = y ~ x) +
    scale_y_continuous(trans = "sqrt") +
    labs(title = "Observed camera-month-row rates through time: wolf_2023",
         x = "deployment start date",
  ...
  month_plot <- ggplot(by_month, aes(month, rate_per_100)) +
    geom_col() +
    labs(title = "Observed monthly wolf encounter frequency: wolf_2023",
  ...
  timing_plot <- ggplot(timing, aes(start_doy, utm_y)) +
    geom_point(alpha = 0.65) +
    geom_smooth(se = TRUE, method = "lm", formula = y ~ x) +
    labs(title = "Deployment timing versus northing: wolf_2023",
...
run_prior_sensitivity <- function(model_dat, settings, family, observed_daily_rate) {
  ...
  cat("\n[wolf_2023] running prior sensitivity\n")
...
model_dat <- load_2023_survey(settings)
...
cat("\n[wolf_2023] running prior-influence screen before sensitivity reruns\n")
...
fit_obj <- fit_2023_model(model_dat, settings, family)

# 8. Final diagnostics from observed-data diagnostic fit with posterior samples.
diag_fit_obj <- fit_2023_diagnostic_model(model_dat, settings, family)

cat(sprintf("\n[wolf_2023] drawing %d posterior samples for PPC and diagnostics from observed-data fit\n", PPC_NSIM))
...
cat(sprintf("\n[wolf_2023] VALIDATION\n"))
cat(sprintf("[wolf_2023]   PPC method: %s\n", diag$ppc_method))
cat(sprintf("[wolf_2023]   Pearson dispersion, model rows: %.3f\n", diag$pearson_disp))
cat(sprintf("[wolf_2023]   Pearson dispersion, camera aggregates: %.3f\n", diag$pearson_disp_camera))
cat(sprintf("[wolf_2023]   PPC pass, camera total / zero / max: %s / %s / %s\n",
            isTRUE(diag$ppc_total_pass),
            isTRUE(diag$ppc_zero_pass),
            isTRUE(diag$ppc_max_pass)))
cat(sprintf("[wolf_2023]   residual Moran's I: %.3f (expected %.3f, two-sided p = %.3f)\n",
            diag$moran_I, diag$moran_expected, diag$moran_p))
cat(sprintf("[wolf_2023]   row PIT KS p: %.3g | camera PIT KS p: %.3g\n",
...
if (!is.null(temporal_diag) && nrow(temporal_diag$lag_summary)) {
  ...
  if (nrow(lag1_console)) {
    cat(sprintf("[wolf_2023]   temporal lag-1 residual correlation: r = %.3f (p = %.3g, n pairs = %d)\n",
...
if (is_zi(family)) {
  cat(sprintf("[wolf_2023]   zero-inflation probability mean: %.3f\n", diag$pi_hat))
}
if (is_nb(family)) {
  cat(sprintf("[wolf_2023]   negative-binomial size mean: %.3f\n", diag$size_hat))
}
cat(sprintf("[wolf_2023]   passes required checks: %s\n", isTRUE(diag$diagnostics_ok)))

if (!isTRUE(diag$diagnostics_ok)) {
  cat(sprintf("[wolf_2023]   NOTE: mapped despite failing %s.\n",
              paste(diagnostic_failures(diag), collapse = "; ")))
  cat("[wolf_2023]         Treat flagged aspects as stated limitations.\n")
}

if (!is.null(model_comparison) && nrow(model_comparison)) {
  cat(sprintf("[wolf_2023]   model comparison written: %s\n",
              path_out(paste0(SURVEY_PREFIX, "_model_comparison.csv"))))
}
if (!is.null(prior_sensitivity) && nrow(prior_sensitivity)) {
  cat(sprintf("[wolf_2023]   prior sensitivity written: %s\n",
              path_out(paste0(SURVEY_PREFIX, "_prior_sensitivity.csv"))))
}
if (!is.null(mesh_sensitivity) && nrow(mesh_sensitivity)) {
  cat(sprintf("[wolf_2023]   mesh sensitivity written: %s\n",
              path_out(paste0(SURVEY_PREFIX, "_mesh_sensitivity.csv"))))
}
cat("\nAll 2023 workflow steps completed in the ordered analysis sequence.\n")
cat("Final outputs are in:\n  ", OUTPUT_DIR, "\n", sep = "")
cat("Key files:\n")
cat("  wolf_2023_ORDERED_WORKFLOW.txt\n")
cat("  wolf_2023_EXPLORATORY_REPORT.txt\n")
cat("  wolf_2023_MODEL_CHOICE_REPORT.txt\n")
cat("  wolf_2023_VALIDATION_REPORT.txt\n")
cat("  wolf_2023_SCIENCE_CHECKS_SUMMARY.txt\n")
cat("  wolf_2023_model_comparison.csv / wolf_2023_MODEL_COMPARISON_REPORT.txt\n")
cat("  wolf_2023_prior_sensitivity.csv / wolf_2023_PRIOR_SENSITIVITY_REPORT.txt\n")
cat("  wolf_2023_mesh_sensitivity.csv / wolf_2023_MESH_SENSITIVITY_REPORT.txt\n")
cat("  wolf_2023_final_predicted_events_per_100_days_mean.tif / _sd.tif\n")
cat("  wolf_2023_final_event_frequency_mean.png / _sd.png\n")
cat("  wolf_2023_nb_spatial_month_posterior_predictive_check.csv\n")
cat("  wolf_2023_nb_spatial_month_model_row_diagnostics.csv\n")
cat("  wolf_2023_nb_spatial_month_camera_residual_diagnostics.csv\n")
cat("  wolf_2023_nb_spatial_month_fitted_scale_sanity_check.csv\n")
cat("  wolf_2023_prior_posterior_*.png / .csv\n")
cat("  wolf_2023_hyperparameters.csv\n")
cat("  wolf_2023_month_coefficients.csv\n")
cat("  wolf_2023_month_observed_summary.csv\n")
cat("  wolf_2023_nb_spatial_month_TEMPORAL_AUTOCORRELATION_REPORT.txt\n")
cat("  wolf_2023_nb_spatial_month_temporal_within_camera_lag_correlation.csv\n")
cat("  wolf_2023_final_spatial_block_cv_summary.csv, if CV was run\n")
cat("  wolf_2023_run_manifest.csv\n")
